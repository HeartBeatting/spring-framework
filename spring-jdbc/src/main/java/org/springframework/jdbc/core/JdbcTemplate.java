...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	public <T> T execute(ConnectionCallback<T> action) throws DataAccessException {		//action回调方法是传入的;
		Assert.notNull(action, "Callback object must not be null");

		Connection con = DataSourceUtils.getConnection(getDataSource());				//Connection对象是实时获取的,不需要外面实例化
		try {

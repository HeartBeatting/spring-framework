...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	private final List<BeanPostProcessor> beanPostProcessors = new ArrayList<BeanPostProcessor>();	//在bean创建时要用到的bean后置处理器
	...
	protected <T> T doGetBean(
			final String name, final Class<T> requiredType, final Object[] args, boolean typeCheckOnly)		// 这里是Spring获取bean/创建bean的核心代码
			throws BeansException {

		final String beanName = transformedBeanName(name);	//获取bean的标准名称
		Object bean;

		// Eagerly check singleton cache for manually registered singletons.
		Object sharedInstance = getSingleton(beanName);		//获取到bean
		if (sharedInstance != null && args == null) {
			if (logger.isDebugEnabled()) {
				if (isSingletonCurrentlyInCreation(beanName)) {		//但是bean正在创建中
			...
			bean = getObjectForBeanInstance(sharedInstance, name, beanName, null);	//获取bean的实例
		}

		else {
			// Fail if we're already creating this bean instance:
			// We're assumably within a circular reference.
			if (isPrototypeCurrentlyInCreation(beanName)) {		// 这里判断beanName是否是正在创建的bean.
			...
			try {
				final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);	// 这里是获取bean definition
				...
				if (mbd.isSingleton()) {
					...
					bean = getObjectForBeanInstance(prototypeInstance, name, beanName, mbd);	//获取bean的实例
				}

				else {	//session, request等 scope
					...
					try {
						...
						bean = getObjectForBeanInstance(scopedInstance, name, beanName, mbd);	//获取bean的实例
					}
	...
	public boolean isTypeMatch(String name, Class<?> targetType) throws NoSuchBeanDefinitionException {
		...
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null) {
			if (beanInstance instanceof FactoryBean) {		//FactoryBean,如果是FactoryBean则需要比较getObjectType;如果是Bean实例,直接比较类型就可以
				if (!BeanFactoryUtils.isFactoryDereference(name)) {
	...
	//spring很多地方都是靠注册BeanPostProcesser 对bean进行包装或者增强
	...
	protected boolean isPrototypeCurrentlyInCreation(String beanName) {
		Object curVal = this.prototypesCurrentlyInCreation.get();	// prototypesCurrentlyInCreation 用于将当前线程正在创建的beanName放进去.
		return (curVal != null &&									// 如果后续有需要注入这些待创建的bean,则说明出现了循环注入.
	...
	protected Object getObjectForBeanInstance(
		...
		if (object == null) {
			...
			if (mbd == null && containsBeanDefinition(beanName)) {	//有beanDefinition

...
public class EnableTransactionManagementIntegrationTests {
	...
	private void assertTxProxying(AnnotationConfigApplicationContext ctx) {
		FooRepository repo = ctx.getBean(FooRepository.class);		//获取bean

		boolean isTxProxy = false;
		if (AopUtils.isAopProxy(repo)) {	//判断如果是代理对象
			for (Advisor advisor : ((Advised)repo).getAdvisors()) {	//找到代理对象所有的拦截器
				if (advisor instanceof BeanFactoryTransactionAttributeSourceAdvisor) {
					isTxProxy = true;		//如果是BeanFactoryTransactionAttributeSourceAdvisor,则表示是事务代理对象
					break;
				}
			}
		}
